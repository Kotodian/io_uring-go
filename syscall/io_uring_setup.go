...
// io_uring features suppored by current kernel version
const (
	IORING_FEAT_SIGNLE_MMAP uint32 = 1 << iota
	IORING_FEAT_NODRP
...
type IOURingParams struct {
	...
	CQEntires    uint32 // when IORING_SETUP_CQSIZE falg is specified
	Flags        uint32 // a bit mast of 0 or more of the IORUING_SETUP_*
	SQThreadCPU  uint32 // when IORING_SETUP_SQPOLL and IORING_SETUP_SQ_AFF flags are specified
	SQThreadIdle uint32 // when IORING_SETUP_SQPOLL flags is specified
	Features     uint32
...
func IOURingSetup(entries uint, params *IOURingParams) (int, error) {
	res, _, errno := syscall.RawSyscall(SYS_IO_URING_SETUP, uintptr(entries), uintptr(unsafe.Pointer(params)), 0)
	if errno != 0 {
